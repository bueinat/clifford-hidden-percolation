...
def handle_circle(g, v, n1, n2):
    if g.type(n1) == zx.VertexType.BOUNDARY and g.type(n2) == zx.VertexType.BOUNDARY:
        g.remove_vertices([v, n1, n2])
        return [n1, n2]
    elif g.type(n1) == zx.VertexType.BOUNDARY and g.type(n2) != zx.VertexType.BOUNDARY:
        g.remove_vertices([v, n1])
        return [n1]
    elif g.type(n1) != zx.VertexType.BOUNDARY and g.type(n2) == zx.VertexType.BOUNDARY:
        g.remove_vertices([v, n2])
        return [n2]
    elif g.type(n1) == g.type(n2):
        ns = g.neighbors(n2)
        ns_filtered = [n for n in ns if n not in (v, n1, n2)]
        g.remove_vertices([v, n2])
        g.add_edges([(n1, n) for n in ns_filtered])
    else:
        g.remove_vertex(v)
        g.remove_edge((n1, n2))
    return []
    ...
    for v in vrlist:
        ...
        if g.type(v) == zx.VertexType.BOUNDARY and v not in g.outputs() and v not in g.inputs():
            ...
            if len(neighbors) == 0:     # remove nodes with no neighbors
                ...
                if len(nn) == 1 and g.type(n) == zx.VertexType.BOUNDARY and n not in g.outputs() and n not in g.inputs():
                    g.remove_vertices([v, n])
                    vrlist.remove(n)
                elif len(nn) == 2 and g.type(n) in [zx.VertexType.X, zx.VertexType.Z]:
                    g.remove_vertex(v)
                elif len(nn) == 1 and nn[0] == v:
                    print("hello")
                    pass
                else:
            ...
            elif len(neighbors) == 2 and neighbors[1] in g.neighbors(neighbors[0]):
                # this is a loop
                removed = handle_circle(g, v, *neighbors)
                [vrlist.remove(i) for i in removed]
            else:
                try:
...
# -> list:
def sample_string_circuit(N, t_factor, p, q, r, u=0, periodic=False, bell_state=False, **kwargs):
    # generate a row
    # print(p, q, r)
    ...
    lgates = ["swap", "lcnot", "rcnot",
              "bell_projection", "id_projection",
              "lphase", "rphase", "lrphase"]
    lprobs = [(1-p) * (1-r), (1-p) * r / 2, (1-p) * r / 2,
              p * q, p * (1 - q) * (1 - u) ** 2,
              p * (1 - q) * (1 - u) * u,  p * (1 - q) * (1 - u) * u,  p * (1 - q) * u ** 2]

    circuit = np.full((total_t + bell_state, N//2), '*' * 15)
    if bell_state:
        circuit[0, :] = "bell_projection"
    for i in range(bell_state, total_t + bell_state, 2):
...
# -> BaseGraph:
def sample_circuit(N, t_factor, p=None, q=None, r=None, u=0, string_circuit=None, apply_state=False, periodic=False):
    # generating stuff
    total_t = int(N * t_factor) // 2 * 2
...
def simplify_circuit(g, quiet=False, simp_method=custom_simp, **kwargs):
    ...
    return simp_method(g, quiet=quiet, **kwargs)
...
def lcnot(g, qubits, q1, q2, t):
    return rcnot(g, qubits, q2, q1, t)


def _add_phase_spider(g, qubits, q, t, phase):
    g.set_type(qubits[2*t + 1, q], zx.VertexType.Z)
    g.set_phase(qubits[2*t + 1, q], phase)
    g.add_edge(
        g.edge(qubits[2*t, q], qubits[2*t + 1, q]), zx.EdgeType.SIMPLE)
    g.add_edge(g.edge(qubits[2*t + 1, q],
                      qubits[2*t + 2, q]), zx.EdgeType.SIMPLE)


def _remove_extra_vertex(g, qubits, q, t):
    g.remove_vertex(qubits[2*t+1, q])
    g.add_edge(g.edge(qubits[2*t, q],
                      qubits[2*t + 2, q]), zx.EdgeType.SIMPLE)


def _phase(g, qubits, q1, q2, t, left, right):
    if left:
        _add_phase_spider(g, qubits, q1, t, 1 / 2)
    if right:
        _add_phase_spider(g, qubits, q2, t, 1 / 2)
    if not left:
        _remove_extra_vertex(g, qubits, q1, t)
    if not right:
        _remove_extra_vertex(g, qubits, q2, t)


def lphase(g, qubits, q1, q2, t):
    _phase(g, qubits, q1, q2, t, True, False)


def rphase(g, qubits, q1, q2, t):
    _phase(g, qubits, q1, q2, t, False, True)


def lrphase(g, qubits, q1, q2, t):
    _phase(g, qubits, q1, q2, t, True, True)
...
# -> Any:
def general_single_iteration(N, t_factor, function, quiet=False, periodic=False, bell_state=False, **kwargs):
    ...
    if 'string_circuit' not in kwargs:
        if 'p' not in kwargs or 'q' not in kwargs or 'r' not in kwargs:
            ...
            kwargs['string_circuit'] = sample_string_circuit(
                N, t_factor, periodic=periodic, bell_state=bell_state, **kwargs)
            if not quiet:
                test_circuit(**kwargs)
    # d = {}
    g, qubits = sample_circuit(
...
def min_cut_hfunction(G, g, **kwargs):
    ...
    # TODO: what do you find out with that exactly?
    return nx.minimum_cut_value(H, super_source, super_target)


def run_all_hfunction(G, g, **kwargs):
    lc, slc = percolation_hfunction(G, g, **kwargs)
    is_path = find_path_hfunction(G, g, **kwargs)
    min_cut_if = min_cut_first(G, g, **kwargs)
    min_cut_ff = min_cut_two_halves(G, g, **kwargs)
    min_cut_Xi = min_cut_X(G, g, **kwargs)
    # min_cut_if, min_cut_ff, min_cut_X = min_cut_hfunction(G, g, **kwargs), min_cut_hfunction(G, g, **kwargs), min_cut_hfunction(G, g, **kwargs)
    # min_cut_if -> min_cut for backwards compitability
    return {'lc': lc, 'slc': slc, 'is_path': is_path, 'min_cut': min_cut_if, 'min_cut_ff': min_cut_ff, 'min_cut_X': min_cut_Xi}
